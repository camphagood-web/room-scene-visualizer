                    ...
                    <div className="flex flex-wrap gap-2">
                        {filterOptions.dateRanges.map((range) => (
                            <button
                                ...
                                className={`px-3 py-1.5 text-sm rounded-lg transition-all ${activeFilters.dateRange === range.id
                    ...
                    <div className="flex flex-wrap gap-2">
                        {filterOptions.roomTypes.map((room) => (
                            <button
                                ...
                                className={`px-3 py-1.5 text-sm rounded-lg transition-all ${activeFilters.roomTypes.includes(room.id)
                    ...
                    <div className="flex flex-wrap gap-2">
                        {filterOptions.designStyles.map((style) => (
                            <button
                                ...
                                className={`px-3 py-1.5 text-sm rounded-lg transition-all ${activeFilters.designStyles.includes(style.id)
                                        ? 'bg-cyan-500 dark:bg-cyan-600 text-white shadow-sm'
                                        : 'bg-stone-200 dark:bg-stone-800 text-stone-600 dark:text-stone-400 hover:bg-stone-300 dark:hover:bg-stone-700'
                                    }`}
                            >
