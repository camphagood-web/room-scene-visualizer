    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-16 py-4 relative min-h-0">
        ...
        <button
          onClick={onPrev}
          disabled={!hasPrev}
          className={`absolute left-2 sm:left-4 p-2 sm:p-3 rounded-full transition-all ${
            hasPrev
        ...
        <div className="max-w-full max-h-full flex items-center justify-center">
          <div
            className={`bg-stone-800 rounded-lg overflow-hidden shadow-2xl ${
              currentSession.aspectRatio === '1:1'
          ...
          >
            ...
            <div className="w-full h-full bg-gradient-to-br from-stone-600 via-stone-700 to-stone-800 relative">
              <div className="absolute inset-6 border border-stone-500/30 rounded" />
              <div className="absolute bottom-6 left-6 right-6 h-1/3 bg-stone-500/20 rounded" />
              <div className="absolute top-1/4 left-1/4 w-1/2 h-1/3 bg-stone-600/30 rounded" />
            </div>
          </div>
        </div>
        ...
        <button
          onClick={onNext}
          disabled={!hasNext}
          className={`absolute right-2 sm:right-4 p-2 sm:p-3 rounded-full transition-all ${
            hasNext
              ? 'bg-white/10 hover:bg-white/20 text-white'
              : 'bg-white/5 text-white/30 cursor-not-allowed'
          }`}
